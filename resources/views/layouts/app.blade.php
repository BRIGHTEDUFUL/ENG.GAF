...
<head>
    ...
    <title>{{ $title ?? 'Ghana Air Force Engineering Command System' }}</title>
...
<div x-data="{
        ...
        currentPage: '{{ request()->routeIs('dashboard') ? 'dashboard' : 'other' }}'
     }"
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div class="flex items-center gap-3 px-5 py-5 border-b border-white/10 shrink-0">
            ...
            <button @click="sidebarCollapsed=!sidebarCollapsed"
                    class="hidden lg:flex ml-auto text-white/50 hover:text-white transition-colors shrink-0">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" :class="sidebarCollapsed ? 'rotate-180' : ''">
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
            ...
            <p x-show="!sidebarCollapsed"
               class="px-3 pt-4 pb-1 text-[9px] font-black text-white/30 uppercase tracking-[0.18em] select-none">
                {{ $groupName }}
            </p>
            ...
            <a href="{{ route($item['route']) }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group
                      {{ $active ? 'bg-white/20 text-white shadow-sm' : 'text-sky-100 hover:bg-white/10 hover:text-white' }}"
               title="{{ $item['label'] }}"
               wire:navigate>
                <svg class="w-5 h-5 shrink-0 transition-transform duration-200 group-hover:scale-110
                           {{ $active ? 'text-white' : 'text-sky-300 group-hover:text-white' }}"
                     fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    {!! $item['icon'] !!}
                </svg>
                <span x-show="!sidebarCollapsed" class="truncate">{{ $item['label'] }}</span>
                @if($active)
                <span x-show="!sidebarCollapsed" class="ml-auto w-1.5 h-1.5 rounded-full bg-white shrink-0"></span>
                @endif
            </a>
            @endif
            @endforeach
        ...
        <div class="px-3 py-4 border-t border-white/10 shrink-0">
            <div class="flex items-center gap-3">
                <div class="flex items-center justify-center w-9 h-9 rounded-xl bg-white/15 text-white text-xs font-bold shrink-0">
                    {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                </div>
                <div class="flex-1 min-w-0 overflow-hidden" x-show="!sidebarCollapsed">
                    <p class="text-white text-xs font-semibold truncate">{{ Auth::user()->name }}</p>
                    <p class="text-sky-300 text-xs truncate">{{ Auth::user()->rank ? Auth::user()->rank.' · ' : '' }}{{ ucfirst(Auth::user()->role) }}</p>
                </div>
                <form method="POST" action="{{ route('logout') }}" x-show="!sidebarCollapsed">
                    @csrf
                    <button type="submit" title="Logout" class="text-sky-300 hover:text-white transition-colors p-1 hover:bg-white/10 rounded-lg">
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden print:overflow-visible">
        ...
        <header class="flex items-center gap-3 h-16 px-4 sm:px-6 bg-white border-b border-sky-100 shadow-sm shrink-0 print:hidden">
            ...
            <div class="hidden lg:flex items-center gap-2 min-w-0">
                ...
                <span class="text-sky-400 text-xs">/</span>
                @isset($heading)
                <h1 class="text-sm font-semibold text-gaf-navy truncate">{{ $heading }}</h1>
                @endisset
            </div>
            ...
            <div class="flex items-center gap-3 ml-auto">
                ...
                @php
                    $roleColors = [
                        ...
                        'commander'  => 'bg-blue-100 text-blue-700 border-blue-200',
                        'supervisor' => 'bg-amber-100 text-amber-700 border-amber-200',
                        'engineer'   => 'bg-green-100 text-green-700 border-green-200',
                        'auditor'    => 'bg-sky-100 text-sky-700 border-sky-200',
                    ];
                    $rc = $roleColors[Auth::user()->role] ?? 'bg-gray-100 text-gray-700';
                @endphp
                <span class="hidden sm:inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border {{ $rc }}">
                    {{ strtoupper(Auth::user()->role) }}
                </span>

                {{-- Avatar --}}
                <div class="flex items-center justify-center w-8 h-8 rounded-xl bg-gaf-gradient text-white text-xs font-bold shadow-sm">
                    {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                </div>
            </div>
        </header>
